...
### KI-9 · 월세 표시 기준과 월세 한도 필터 기준 불일치 🟢 해결 (2026-06-03)
...
- **해결(2026-06-03)**: 서버가 `rentDepositManwon`(보증금 중위)·`rentPureMonthlyManwon`(순수 월세 중위)을
  후보→응답까지 노출(`RegionMetrics`/route mapping/client type). 카드 대표가를 거래유형별로 분리:
  - **MONTHLY** → "월세 X만"(=순수 월세, **월세 한도 필터와 동일 기준**) + 보조 "보증금 Y".
  - **JEONSE** → "보증금 X억"(=전세금, **보증금 한도 필터와 동일**). 기존 "전세 월 X만"(환산) 제거.
  - **SALE/표본부족(sale-proxy)** → 기존 "가격 X억"(매매가) 유지.
  RIR·"주거비 %"는 그대로 합산 월주거비(`monthlyHousingCost`) 기준 — 별개 개념이라 미변경. typecheck OK(client·server).
...
- **해결(2026-06-03)**: 게이트가 `budgetFilteredBreakdown{deposit, monthlyRent, salePrice}` 로 사유별 집계
  (첫 실패 사유에서 continue → 상호배타·중복없음). meta 로 전달 → 클라 배너가
  "보증금 초과 N · 월세 초과 M 숨김" 식으로 분리 표기(breakdown 없으면 단일 문구 폴백).
  추가로 **월세만 초과 시 "월세 한도 늘리기"** 버튼이 `monthlyRentCap` 을 올리도록 분기(기존엔 항상 예산만 상향).
...
### KI-26 · 통근 폴백 — 지하철-only 내부 라우터가 수도권(버스 의존)에서 한계 🟡 현 상태 유지·v2 멀티모달 (2026-06-04)
- **배경**: ODsay 캐시 미스 통근분 폴백을 직선거리 추정 → **내부 지하철 라우터(쿼터0)** 비대칭 하이브리드로 배선
  (`subwayRouterSingleton`, 라우터가 직선보다 *느릴 때만* 채택 + 근거리 임계 18분). 상세: `server/doc/2026-06-04/odsay-commute-routing-analysis.md` §7.4~7.6.
- **증상(실측, ODsay 540표본 권역 분리)**: **서울 동**은 라우터로 개선(직선 MAE 9.6→7.8)이나 **수도권 동**은 악화(10.9→11.4).
  수도권은 **버스 의존도가 높아** 지하철-only 라우터가 경로를 억지 우회로 과대추정(+ 그래프 듬성). 라우터 이득은
  **직선이 심하게 낙관적인 철도 corridor**(부천 bias −12.7·강남 −5.8 = 삼산동류)에 한정.
- **결정(사용자 합의)**: **현 상태 유지**. 근거 ① 수도권 악화 +0.5분으로 작음 ② 비대칭 가드라 라우터는 항상
  "느리게=강등" 방향(거짓 강등 ≪ 거짓 승격 피해) ③ corridor 이득 실재 → MVP 리스크 낮음. `COMMUTE_ROUTER=off` 로 비활성 가능.
- **잔여(v2)**: 버스 멀티모달은 배차·지연·교통량 가변이 커 ROI 낮아 **보류**. v2 후보로 **호재공시·재개발/상권 LSTM 가변축**이
  더 실용적(사용자 제안). 그 외 게이트 FN 구제(판교류 직통 동)·수도권 약칭역명 그래프 보강은 §7.6 잔여 참조.
- **연관**: KI-7(통근 표시·랭킹) · KI-22(통근 게이트) · KI-23(ODsay 쿼터 폴백).
