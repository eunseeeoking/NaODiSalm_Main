...
async function main() {
  ...
  // 동 centroid 조회 — t_legal_dong(법정동) × **4종 단지 UNION**(아파트+오피+빌라+단독) 이름조인, 좌표 AVG.
  //  (2026-06-XX KI-19 후속: serving(fetchRegionAggregates)은 4종 union universe인데 시드가 apt-only면
  //   빌라·오피만 있는 동(특히 수도권)이 POI 미적재 → life 추정. 시드 universe를 serving과 동일하게 4종으로 맞춤.)
