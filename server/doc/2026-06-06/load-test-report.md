...
- **무료 티어(0.1 CPU)는 `connection_limit` 으로도 못 살림** — 동시 20은 27s(사실상 불가)·30에서 완전 붕괴 → 발표엔 Standard 필수.
...
|---|---|---|---|---|---|
| 2 | 100% / 3.95s | 100% / 2.97s | 90% / 1.3s | **100% / 1.5s** | 100% / 3.1s |
| 10 | 100% / 13.5s | 100% / 14.0s | 60% / 6.3s | **100% / 5.4s** | 100% / 13.7s |
| 20 | **0%** (timeout) | 100% / 27.9s | 80% / 8.2s | **100% / 11.3s** | 100% / 27.0s |
| 30 | **0%** | **0%** (timeout) | 83% / 11.9s | **100% / 15.5s** | **0%** (timeout) |
| 50 | **0%** (t43+500×7) | **0%** (t19+500×31) | 90% / 20.4s | **100% / 26.0s** | **0%** (timeout) |
| 처리량 | ~0.7 req/s | ~0.7 req/s | ~1.9 req/s | **~1.9 req/s** | ~0.7 req/s |

⑤ 재측정(빌드 완료 확인 후, 콜드스타트 10.4s). **free+풀20 ≈ free 기본풀(②)** — `connection_limit` 은 free 0.1 CPU 에선 거의 무효(CPU가 병목이라 풀을 소진할 만큼 동시 쿼리를 못 돌림; 풀 효과는 ④처럼 CPU 충분할 때만). free 는 동시 20을 27s 로 턱걸이(사실상 불가)·**30에서 완전 붕괴**. (초기 측정의 conc20 0%·503 은 다운그레이드 전환 중 인스턴스 재시작 오염이었음 → 본 표는 재측정값.)
...
- 처리량이 동시성과 무관하게 **0.7 req/s에 고정** = 직렬 자원 포화(CPU). free에선 동시 10이 ~14s, 20은 27s(턱걸이), 30+ 붕괴.
